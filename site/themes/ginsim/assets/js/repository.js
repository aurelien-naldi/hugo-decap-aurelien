function load_repository(target, uid) {
    var parent = document.getElementById(uid);
    parent.innerHTML = "... Loading data into the repository ...";
    var xhr = new XMLHttpRequest();
    xhr.responseType = 'json';
    xhr.onreadystatechange = function () {
          if (xhr.readyState == 4 && xhr.status == "200") {
            build_repository(xhr.response, uid);
          }
    };
...
function build_repository(tabledata, uid) {
    var repo = new Repository(uid, tabledata);
}
...
class Repository {
    ...
    update_filter() {
        ...
        var s = this.in_filter.value;
        var allfilters = s.match(/(?:[^\s"]+|"[^"]*")+/g);
        var allregex = new Array(0);
        for (var curfilter in allfilters) {
            allregex.push(new RegExp(allfilters[curfilter], "i"));
        }

        loopm:
            for (var m in this.data) {
                for (var curfilter in allfilters) {
                    var regexp = allregex[curfilter];
                    try {
...
//                        alert('error: '+error);
    ...
    fill() {
        ...
        for (var m of this.filtered.slice(start,end)) {
            var entry = this.data[m];
            var eelt = document.createElement("div");
            eelt.classList.add("entry");
            eelt.append(format_entry(entry));
            this.container.append(eelt);
        }
    }
}
...
function value_matches_filter(v, regexp) {
    if (v) {
        return v.search(regexp) >= 0;
    }
    return false;
}

function array_matches_filter(a, regexp) {
    if (a) {
        for (idx in a) {
            if (value_matches_filter(a[idx], regexp)) return true;
        }
    }
    return false;
}
